...
from teacher_model.graph_met_network import GraphMETNetwork
...
def resolution(weights, prediction, truth, batch):
    ...
    if has_pfmet:
        resolutions['pfMET'] = compute(v_pfMET)
        resolutions['puppiMET'] = compute(v_puppiMET)
    if has_deepmet:
        resolutions.update({
